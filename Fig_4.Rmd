...
mdat_wgbs <- read.csv(file = "wgbs/metadata/wgbs_paths_peb_servers.csv")
mdat_wgbs <- mdat_wgbs[mdat_wgbs$donor == "MEL1", ]
mdat_wgbs$CG_bsobj_local <- str_c("wgbs/CG_bsobj/", basename(mdat_wgbs$CG_bsobj))
lapply(mdat_wgbs$CG_bsobj_local, file.exists)
...
read_dmr <- function(dmr_rds){
    ...
    g1_mCG <- make_mC_matrix(obj_fls = mdat_wgbs$CG_bsobj_local[mdat_wgbs$library %in% g1_libs], 
                             gr=dmr_gr)
    g2_mCG <- make_mC_matrix(obj_fls = mdat_wgbs$CG_bsobj_local[mdat_wgbs$library %in% g2_libs], 
                             gr=dmr_gr)
...
write.xlsx(x = dmr_df_list, file = "supplementary_tables/MEL1_CG_DMR_results.xlsx",
           append=TRUE, row.names=FALSE, sheetName=names(dmr_grl))
```
...
gene_down_gr <- gene_gr[gene_gr$gene_id %in% gene_down] %>%
    resize(width = 2000, fix = "start")

gene_up_gr <- gene_gr[gene_gr$gene_id %in% gene_up] %>%
    resize(width = 2000, fix = "start")
...
Run the enrichment tests (on server)
```{r, eval=FALSE}
...
# Put limit z-scores for colours on heatmap
...
icr_matrix <- make_mC_matrix(obj_fls = mdat_wgbs$CG_bsobj_local, gr = icr_gr)
...
pdf("methylCseq/plots/MEL1_germline_ICR_methylation_heatmap.pdf", width = 5, height = 6)
pheatmap(icr_matrix,
               show_rownames = TRUE, border_color = NA,
         fontsize = 5, main="Imprint control regions",
         annotation_row = annot_row)
...
calc_icr_delta <- function(group){
    ...
    dat <- icr_means[ ,group] - icr_means[ ,"ESC"] %>% as.numeric()
    dat <- data.frame(Group=group, mCG_delta=dat, Gene=rownames(icr_means))
    
}

icr_delta_dat <- lapply(c("Primed", "TNT", "NtP"), calc_icr_delta) %>%
    do.call(rbind, .)
...
gene_df_imprint$id <- str_c(gene_df_imprint$contrast, gene_df_imprint$gene_id, sep = "_")
...
icr_merge_df$contrast <- factor(icr_merge_df$contrast, levels=c("Primed", "TNT", "NtP"))
...
## Motif analysis of differential atac peaks
```{r}
...
## Footprinting of differential ATAC peaks
### Run on server

### Get BED files for enriched motifs

Interset motif beds with DE peaks
```{bash}
bedtools intersect -wa \
-a /home/sbuckberry/working_data_04/hs-reprogram/atac/aligned_data/ESC_vs_Primed/POU5F1_MA1115.1/beds/POU5F1_MA1115.1_all.bed \
-b /home/sbuckberry/working_data_04/hs-reprogram/atac/all_differential_peaks_primed_down.bed \
> /home/sbuckberry/working_data_04/hs-reprogram/atac/tobias_out/differential_peaks_primed_down_POU5F1_MA1115_1_all.bed

bedtools intersect -wa \
-a /home/sbuckberry/working_data_04/hs-reprogram/atac/aligned_data/ESC_vs_Primed/KLF4_MA0039.4/beds/KLF4_MA0039.4_all.bed \
-b /home/sbuckberry/working_data_04/hs-reprogram/atac/all_differential_peaks_primed_up.bed \
> /home/sbuckberry/working_data_04/hs-reprogram/atac/tobias_out/differential_peaks_primed_up_KLF4_MA0039_4_all.bed

bedtools intersect -wa \
-a /home/sbuckberry/working_data_04/hs-reprogram/atac/aligned_data/ESC_vs_Primed/CTCF_MA0139.1/beds/CTCF_MA0139.1_all.bed \
-b /home/sbuckberry/working_data_04/hs-reprogram/atac/all_differential_peaks_primed_up.bed \
> /home/sbuckberry/working_data_04/hs-reprogram/atac/all_differential_peaks_primed_up_CTCF_MA0139.1_all.bed


bedtools intersect -wa \
-a /home/sbuckberry/working_data_04/hs-reprogram/atac/aligned_data/ESC_vs_Primed/NFYA_MA0060.3/beds/NFYA_MA0060.3_all.bed \
-b /home/sbuckberry/working_data_04/hs-reprogram/atac/all_differential_peaks_primed_up.bed \
> /home/sbuckberry/working_data_04/hs-reprogram/atac/all_differential_peaks_primed_up_NFYA_MA0060.3_all.bed


/home/sbuckberry/working_data_04/hs-reprogram/atac/aligned_data/ESC_vs_Primed/POU5F1_MA1115.1/beds/POU5F1_MA1115.1_ESC_bound.bed
```
...
cd /home/sbuckberry/working_data_04/hs-reprogram/atac/aligned_data/

TOBIAS PlotAggregate --plot_boundaries --share-y both --flank 500 \
--TFBS /home/sbuckberry/working_data_04/hs-reprogram/atac/tobias_out/differential_peaks_primed_down_POU5F1_MA1115_1_all.bed \
--signals PSC_ESC_all_merge_corrected_footprints.bw \
PSC_Primed_all_merge_corrected_footprints.bw \
PSC_TNT_all_merge_corrected_footprints.bw \
PSC_NtP_all_merge_corrected_footprints.bw \
--signal-labels ESC Primed TNT NtP \
--output /home/sbuckberry/working_data_04/hs-reprogram/atac/tobias_out/differential_peaks_primed_down_POU5F1_flank_500.pdf

TOBIAS PlotAggregate --plot_boundaries --share-y both --flank 100 \
--TFBS /home/sbuckberry/working_data_04/hs-reprogram/atac/tobias_out/differential_peaks_primed_down_POU5F1_MA1115_1_all.bed \
--signals PSC_ESC_all_merge_corrected_footprints.bw \
PSC_Primed_all_merge_corrected_footprints.bw \
PSC_TNT_all_merge_corrected_footprints.bw \
PSC_NtP_all_merge_corrected_footprints.bw \
--signal-labels ESC Primed TNT NtP \
--output /home/sbuckberry/working_data_04/hs-reprogram/atac/tobias_out/differential_peaks_primed_down_POU5F1_flank_100.pdf


TOBIAS PlotAggregate --plot_boundaries --share-y signals --flank 30 \
--TFBS /home/sbuckberry/working_data_04/hs-reprogram/atac/tobias_out/differential_peaks_primed_up_KLF4_MA0039_4_all.bed \
--signals PSC_ESC_all_merge_corrected_footprints.bw \
PSC_Primed_all_merge_corrected_footprints.bw \
PSC_TNT_all_merge_corrected_footprints.bw \
PSC_NtP_all_merge_corrected_footprints.bw \
--signal-labels ESC Primed TNT NtP \
--output /home/sbuckberry/working_data_04/hs-reprogram/atac/tobias_out/differential_peaks_primed_up_KLF4_MA0039_4.pdf

TOBIAS PlotAggregate --plot_boundaries --share-y both --flank 150 \
--TFBS /home/sbuckberry/working_data_04/hs-reprogram/atac/all_differential_peaks_primed_up_CTCF_MA0139.1_all.bed \
--signals PSC_ESC_all_merge_corrected_footprints.bw \
PSC_Primed_all_merge_corrected_footprints.bw \
PSC_TNT_all_merge_corrected_footprints.bw \
PSC_NtP_all_merge_corrected_footprints.bw \
--signal-labels ESC Primed TNT NtP \
--output /home/sbuckberry/working_data_04/hs-reprogram/atac/tobias_out/differential_peaks_primed_up_CTCF_MA0139.pdf


TOBIAS PlotAggregate --plot_boundaries --share-y both \
--TFBS /home/sbuckberry/working_data_04/hs-reprogram/atac/all_differential_peaks_primed_up_NFYA_MA0060.3_all.bed \
--signals PSC_ESC_all_merge_corrected_footprints.bw \
PSC_Primed_all_merge_corrected_footprints.bw \
PSC_TNT_all_merge_corrected_footprints.bw \
PSC_NtP_all_merge_corrected_footprints.bw \
--signal-labels ESC Primed TNT NtP \
--output /home/sbuckberry/working_data_04/hs-reprogram/atac/tobias_out/all_differential_peaks_primed_up_NFYA_MA0060.3_all.pdf

git add /home/sbuckberry/working_data_04/hs-reprogram/atac/tobias_out/*.pdf && git commit -m 'plot updates' && git pull && git push
```


```{r}

TOBIAS PlotAggregate --share_y both --plot_boundaries --signal-on-x \
--TFBS /home/sbuckberry/working_data_04/hs-reprogram/atac/aligned_data/ESC_vs_Primed/POU5F1_MA1115.1/beds/POU5F1_MA1115.1_ESC_bound.bed \
--signals PSC_ESC_all_merge_corrected_footprints.bw \
PSC_Primed_all_merge_corrected_footprints.bw \
PSC_TNT_all_merge_corrected_footprints.bw \
PSC_NtP_all_merge_corrected_footprints.bw \
--signal-labels ESC Primed TNT NtP \
--output /home/sbuckberry/working_data_04/hs-reprogram/atac/tobias_out/differential_esc_bound_POU5F1.pdf
...
mdat_wgbs <- read.csv("wgbs/metadata/wgbs_paths_peb_servers.csv")
mdat_wgbs_MEL1 <- mdat_wgbs[mdat_wgbs$donor == "MEL1", ] 
...
icr_matrix <- make_mC_matrix(obj_fls = CG_obj_list, gr = icr_gr, cores = 3)
